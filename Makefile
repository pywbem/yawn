...
# Names of the Apache modules:
mod1_name := mod_python
mod2_name := mod_wsgi
...
package_py_files := \
    $(wildcard $(mod1_name)/*.py) \
    $(wildcard $(mod1_name)/*/*.py) \
		$(wildcard $(mod1_name)/*/*/*.py) \
		$(wildcard $(mod2_name)/*.py) \
    $(wildcard $(mod2_name)/*/*.py) \
		$(wildcard $(mod2_name)/*/*/*.py) \
...
.PHONY: develop
develop: Makefile install_reqs_$(python_mn_version).done develop_reqs_$(python_mn_version).done
# ifdef TEST_INSTALLED
...
.PHONY: all
all: develop check pylint  # build builddoc installtest test
	@echo "Makefile: Target $@ done."
